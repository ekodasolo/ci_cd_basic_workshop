...
3. cities-api-dev / cities-api-prod スタック   - パイプラインがデプロイしたCFNスタック
...
2. 左ナビ：**Pipelines** で以下を1つずつ削除します
   - `cities-api-pipeline-dev`
   - `cities-api-pipeline-prod`
...
2. 左ナビ：**Build projects** で以下を削除します
   - `cities-api-build-dev`
   - `cities-api-build-prod`
3. 各プロジェクトを選択し、右上の **Delete build project** → 確認ダイアログで名前を入力 → **Delete**

## 3. アプリケーション用CloudFormationスタックを削除する

`cities-api-dev` と `cities-api-prod` スタックを削除すると、Lambda関数・API Gateway・実行ロール等が一括削除されます。
...
2. バケット一覧から `cities-api-artifacts-<AccountID>-<Region>` を選択
3. 右上の **Empty** をクリック
...
1. AWSマネジメントコンソールで **CloudFormation** を開きます
2. 事前構築スタック（例：`cities-api-setup` など、講師が指定した名前）を選択
...
| `cities-api-dev` スタック削除が `Resource cannot be deleted` で失敗 | Lambda実行ロールに何かがアタッチされている | CloudFormation 画面でエラー詳細を確認、必要に応じて手動でリソースを削除してから再試行 |
